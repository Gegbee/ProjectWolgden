	...
	for row in image.get_height():
		for column in image.get_width():
			...
			if image.get_pixel(column, row) == ColorN("red"):
				...
				if image.get_pixel(column + 1, row) == ColorN('white'):
					print('door at: ', column, row) 
				if image.get_pixel(column - 1, row) == ColorN('white'):
					print('door at: ', column, row) 
				if image.get_pixel(column, row + 1) == ColorN('white'):
					print('door at: ', column, row) 
				if image.get_pixel(column + 1, row - 1) == ColorN('white'):
					print('door at: ', column, row) 
