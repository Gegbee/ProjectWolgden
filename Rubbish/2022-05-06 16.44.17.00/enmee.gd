...
func look(delta):
	...
	setHandDir(player_vars.position - global_position)
	dir = find_target()
...
func find_target():
	var player_cast = $PlayerCast
	player_cast.cast_to = (player_vars.position - global_position)
...
#		state = FIGHT
  ...
  # if we can see the target, chase it
	if player_cast.is_colliding() == false:
		dir = player_cast.cast_to.normalized()
		return dir

  # or chase first scent we can see	
	else:
		var scent_cast = $TrailCast
		scent_cast.cast_to = (player_vars.position - global_position)
		scent_cast.force_raycast_update()
	
		for scent in player_vars.scent_trail:
			scent_cast.cast_to = (scent.position - global_position)
			scent_cast.force_raycast_update()
			if scent_cast.is_colliding() == false:
				dir = scent_cast.cast_to.normalized()
				return dir
				break
...
func fight(delta):
	...
	setHandDir(player_vars.position - global_position)
	var look = get_node("RayCast2D")
	look.cast_to = (player_vars.position - global_position)
	look.force_raycast_update()
	
	var length = player_vars.position - global_position
	item_in_use = false
	if length.length() > 15:
		useHand("left")
		state = CHASE
